...
public class YouTubeController : ControllerBase
{
    ...
    public async Task<IActionResult> ImportYouTubeVideo(string feedId, [FromBody] YouTubeImportRequest request)
    {
        ...
        // Validate format
        if (!string.Equals(request.Format, "audio", StringComparison.OrdinalIgnoreCase) &&
            !string.Equals(request.Format, "video", StringComparison.OrdinalIgnoreCase))
        {
            return BadRequest(new { error = "Format must be 'audio' or 'video'" });
        }
        ...
        var metadata = await _ytDlpService.GetMetadataAsync(request.Url, HttpContext.RequestAborted);
        if (metadata == null)
        {
            return BadRequest(new { error = "Video is unavailable, private, or age-restricted" });
        }

        // Generate IDs
        var jobId = Guid.NewGuid().ToString("N");
        var format = request.Format.ToLowerInvariant();
        var episodeId = Episode.GenerateYouTubeId(feedId, videoId, format);

        // Create job status in Table Storage
        var fileName = $"{videoId}.{(string.Equals(format, "video", StringComparison.OrdinalIgnoreCase) ? "mp4" : "m4a")}";
