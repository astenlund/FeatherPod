...
public sealed class UserService : IUserService, IDisposable
{
    ...
    public async Task<User?> GetUserByApiKeyAsync(string apiKey)
    {
        ...
        try
        {
            ...
            {
                var keyHash = ApiKeyGenerator.Hash(apiKey, user.ApiKeySalt);
    ...
    public async Task<string> CreateUserAsync(User user)
    {
        ...
        var (apiKey, salt) = ApiKeyGenerator.Generate(user.Id);
        var keyHash = ApiKeyGenerator.Hash(apiKey, salt);
    ...
    {
        var (newApiKey, newSalt) = ApiKeyGenerator.Generate(userId);
        var newKeyHash = ApiKeyGenerator.Hash(newApiKey, newSalt);
